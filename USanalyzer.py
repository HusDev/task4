...
def count_occurrence(row_num):
    array = []
    with open('US/USvideos.csv','r',newline='') as f:
      reader = csv.reader(f)
...
data = pd.read_csv("US/USvideos.csv")
...
by_likes.to_csv("US/reactions/by_likes.csv")
by_dislikes.to_csv("US/reactions/by_dislikes.csv")
by_comments.to_csv("US/reactions/by_comments.csv")
by_ratio.to_csv("US/reactions/by_ratio.csv")

# STEP2
# Exposure
by_views = c_data.sort_values("views")
by_views.views.to_csv("US/exposure/by_views.csv")
...
with open('US/exposure/by_streaks.csv','w',newline='') as new_file:
...
#STEP3 
# Category
file = open("US/USvideos.csv",newline='')
...
with open('US/category/by_category.csv','w',newline='') as new_file:
...
by_mentions.to_csv("US/tags/by_mentions.csv",index=False)

tag_by_cat = c2_data.sort_values("category_id")
tag_by_cat = tag_by_cat.loc[:,["category_id","tags"]]
tag_by_cat.to_csv("US/tags/tag_by_cat.csv")
...
tags_by_likes.to_csv("US/tags/tags_by_likes.csv")

tags_by_views=tags_data.sort_values("views")
tags_by_views.to_csv("US/tags/tags_by_views.csv")